...
async function requestStream(track, type, manager) {
    if (type === "SoundCloud") {
        return soundcloud_downloader_1.default.download(track.id).catch((_) => undefined);
    }
    else if (type === "LocalFile") {
        return (0, fs_1.createReadStream)(track.id);
    }
    else if (type === "Url") {
        return (await (0, undici_1.request)(track.id)).body.arrayBuffer();
    }
    else if (type === "Youtube") {
        ...
        if (!track.id) {
            ...
            return yt.download(data.videos.as(Video_1.default)[0].id, {
