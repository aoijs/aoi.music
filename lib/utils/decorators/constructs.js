...
exports.constructTrack = exports.constructCache = exports.constructManager = exports.decorateConstructor = void 0;
...
function constructCache() {
    return decorateConstructor((manager, args) => {
        const config = args[0];
        if (!("cacheType" in config) || !enums_1.CacheType[config.cacheType])
            throw new Error(`Cache Type is invalid`);
        if (config.cacheType === enums_1.CacheType.Disk && (typeof config.directory !== "string" || !config.directory))
