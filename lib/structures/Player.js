...
class Player {
    constructor(data) {
        ...
        this._state = enums_1.PlayerStates.Idling;
    ...
    get state() {
        return enums_1.PlayerStates[this._state];
    }
    set state(n) {
        if (this.state === n)
            return;
        this._state = enums_1.PlayerStates[n];
    }
    /**
     * search
     */
    async search(query, type, limit = 1) {
        ...
        if (type === 0) {
            for (let i = 0; i < urls.length; i++) {
                ...
                if (this.queue.list.length === 1 && !this.queue.current) {
                    ...
                    this.manager.emit(enums_1.PlayerEvents.QUEUE_START, urls, this.textChannel);
    ...
    play() {
        ...
        this.manager.emit(enums_1.PlayerEvents.TRACK_START, this.queue.current, this.textChannel);
    ...
    _configPlayer() {
        this.player.on("stateChange", async (os, ns) => {
            if (os.status !== voice_1.AudioPlayerStatus.Idle &&
                ...
                if (this.options.paused) {
                ...
                else if (this.options.mode === enums_1.LoopMode.Track && this.queue.current) {
                    this.manager.emit(enums_1.PlayerEvents.TRACK_END, this.queue.current, this.textChannel);
                    this._playSingleTrack();
                }
                else if (this.options.mode === enums_1.LoopMode.Queue &&
                    ...
                    this.manager.emit(enums_1.PlayerEvents.TRACK_END, this.queue.current, this.textChannel);
                    this.manager.emit(enums_1.PlayerEvents.QUEUE_END, this.textChannel);
        ...
        this.player.on("error", async (error) => {
            this.manager.emit(enums_1.PlayerEvents.AUDIO_ERROR, error, this.textChannel);
        });
        if (this.debug) {
    ...
    async _destroyPlayer() {
        if (this.manager.config.cache.cacheType === "Disk") {
            if ((0, fs_1.existsSync)(`.${this.manager.config.cache.directory}/${this.textChannel.guildId}`)) {
                await (0, promises_1.rm)(`.${this.manager.config.cache.directory}/${this.textChannel.guildId}`, {
                    recursive: true,
                    force: true,
                });
            }
        ...
        this.options.mode = enums_1.LoopMode.None;
    ...
    pause() {
        ...
        this.manager.emit(enums_1.PlayerEvents.TRACK_PAUSE, this.textChannel);
    }
    resume() {
        this.options.paused = false;
        this.player.unpause();
        this.manager.emit(enums_1.PlayerEvents.TRACK_RESUME, this.textChannel);
    }
    ...
    shuffleQueue() {
        ...
        this.queue.list = (0, helpers_1.shuffle)(this.queue.list.slice(1, -1));
    ...
    skipTo(number) {
        if (this.options.mode != enums_1.LoopMode.Queue) {
