...
let CacheManager = class CacheManager {
    constructor(config) {
        this.config = {
            limit: Infinity,
            directory: "music",
            cacheType: enums_1.CacheType.Memory,
            enabled: true,
        };
        this.map = new Map();
        this.config = config;
    ...
    async write(id, stream, type, guildId) {
        ...
        if (type === 1)
            return;
        if (this.config?.cacheType === enums_1.CacheType.Memory) {
            const data = await this._convertStreamToBuffer(stream);
            this.map.set(id, data);
            return;
        }
        else if (this.config?.cacheType === enums_1.CacheType.Disk) {
            ...
            this.map.set(id, `${this.config.directory}/${guildId}/` +
                id
                    .replaceAll("/", "#SLASH#")
                    .replaceAll(":", "#COLON#")
                    .replaceAll("?", "#QUESTION#"));
            fs.writeFileSync(`${this.config.directory}/${guildId}/` +
                id
                    .replaceAll("/", "#SLASH#")
                    .replaceAll(":", "#COLON#")
                    .replaceAll("?", "#QUESTION#"), "");
            const st = fs.createWriteStream(`${this.config.directory}/${guildId}/` +
                id
                    .replaceAll("/", "#SLASH#")
                    .replaceAll(":", "#COLON#")
                    .replaceAll("?", "#QUESTION#"));
    ...
    get(id, guildId) {
        ...
        if (this.config?.cacheType === enums_1.CacheType.Disk) {
            let st;
            try {
                st = fs.createReadStream(`${this.config.directory}/${guildId}/${id
                    .replaceAll("/", "#SLASH#")
                    .replaceAll(":", "#COLON#")
                    .replaceAll("?", "#QUESTION#")}`);
            }
