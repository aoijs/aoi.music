...
let Manager = class Manager extends tiny_typed_emitter_1.TypedEmitter {
    constructor(config) {
        ...
        this.config = config;
        this.searchManager = new Search_1.Search({ clientId: this.config.soundcloud.clientId });
    ...
    async joinVc({ voiceChannel, textChannel, debug = false }) {
        ...
        try {
            ...
            this.players.set(voiceChannel.guildId, new Player_1.default({ connection, voiceChannel, textChannel, manager: this, debug: true }));
        }
        catch (error) {
            connection.destroy();
