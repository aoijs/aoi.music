...
let Manager = class Manager extends tiny_typed_emitter_1.TypedEmitter {
    constructor(config) {
        if (config.soundcloud.likeTrackLimit &&
            config.soundcloud.likeTrackLimit > 350) {
            throw new Error("[MANAGER](SoundCloudError) Like Track Request Limit is too high, please lower it to 350 or less");
        }
        super();
     ...
     * joinVc
     */
    async joinVc({ voiceChannel, textChannel, selfDeaf = true, selfMute = false, debug = false, }) {
        const data = {
