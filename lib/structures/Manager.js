...
let Manager = class Manager extends tiny_typed_emitter_1.TypedEmitter {
    constructor(config) {
        ...
        this.providers = {
            twitch: new source_1.TwitchProvider(),
            soundcloud: new source_1.SoundcloudProvider({ clientId: undefined }),
        };
        this.config = config;
        this.searchManager = new Search_1.Search({
            clientId: this.config.soundcloud.clientId,
        });
        this.cacheManager = new Cache_1.default(this.config.cache || { enabled: true, cacheType: constants_1.CacheType.Memory });
