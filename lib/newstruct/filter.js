...
class Filter {
    ...
    async seek(time, player) {
        ...
        const stream = await (0, request_1.requestStream)(track, track.formattedPlatforms, player.options.manager);
        let str;
...
_Filter_config = new WeakMap(), _Filter_instances = new WeakSet(), _Filter_apply = async function _Filter_apply(player) {
    ...
    newResource.playbackDuration = __classPrivateFieldGet(this, _Filter_config, "f").filterFromStart ? 0 : r.playbackDuration;
