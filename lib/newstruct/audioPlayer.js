...
class AudioPlayer {
    ...
    async play(emit = true) {
        ...
        let stream;
        //@ts-ignore
        stream = await (0, request_1.requestStream)(current, current.formattedPlatforms, this.options.manager);
        let s;
            ...
            if (os.status !== voice_1.AudioPlayerStatus.Idle &&
                ...
                if (__classPrivateFieldGet(this, _AudioPlayer_modes, "f").paused) {
                    ...
                    if (__classPrivateFieldGet(this, _AudioPlayer_modes, "f").filtering) {
                        ...
                        // this._destroy();
                        // reset queue length to trigger queuestart on new tracks?
                        this.queue.length = 0;
                    }
                }
            }
    ...
    async add(track, type, member) {
        for (let i = 0; i < track.length; i++) {
            if (type === enums_1.PlatformType.Youtube) {
                ...
                const previous = this.queue?.[__classPrivateFieldGet(this, _AudioPlayer_modes, "f").currentTrack - 1];
    ...
    async autoPlayNext() {
        if (this.autoPlay ===
            enums_1.AutoPlay.Youtube /*|| this.autoPlay === AutoPlay.Spotify*/) {
            ...
            if (this.currentTrack.formattedPlatforms.toLowerCase() ===
                enums_1.AutoPlay.Youtube /*|| this.currentTrack.formattedPlatforms.toLowerCase() === AutoPlay.Spotify*/) {
                const parsed = await (await (0, undici_1.fetch)(`https://youtube.com/watch?v=${this.currentTrack.id}`)).text();
                const data = (0, helpers_1.ytRelatedHTMLParser)(parsed);
                const ids = (0, helpers_1.YoutubeRelated)(data);
                for (const id of ids) {
                    const info = await (0, request_1.requestInfo)(id, this.currentTrack.formattedPlatforms.toLowerCase() === "youtube"
                        ? "Youtube"
            ...
            else if (this.currentTrack.formattedPlatforms.toLowerCase() ===
