...
class AudioPlayer {
    ...
    async play() {
        ...
        resource.volume.setVolume(__classPrivateFieldGet(this, _AudioPlayer_modes, "f").volume);
    ...
    async autoPlayNext() {
        if (this.autoPlay === enums_1.AutoPlay.Youtube ||
            ...
            if (this.currentTrack.formatedPlatforms.toLowerCase() ===
                ...
                const ids = (0, helpers_1.YoutubeRelated)(data);
                for (const id of ids) {
                    const info = await (0, request_1.requestInfo)(id, this.currentTrack.formatedPlatforms.toLowerCase() === "youtube" ? "Youtube" : "Spotify", this.options.manager);
                    if (!info)
                        continue;
                    this.queue.push({
                        ...info,
