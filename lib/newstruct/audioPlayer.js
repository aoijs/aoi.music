...
class AudioPlayer {
    ...
    async autoPlayNext() {
        if (this.autoPlay === enums_1.AutoPlay.Youtube ||
            ...
            if (this.currentTrack.formatedPlatforms.toLowerCase() ===
                ...
                const yt = await this.options.manager.platforms.youtube;
                const data = (0, helpers_1.ytRelatedHTMLParser)(await (await (0, undici_1.fetch)(`https://youtube.com/watch?v=${this.currentTrack.id}`)).text());
                const ids = (0, helpers_1.YoutubeRelated)(data);
                for (const id of ids) {
                    const info = await (0, request_1.requestInfo)(id, constants_1.formatedPlatforms[this.autoPlay], this.options.manager);
                    if (!info)
                        continue;
                    this.queue.push({
                        ...info,
    ...
    getQueue(page = 1, limit = 10, format = "{position}) {title} | {requester.user.name}") {
