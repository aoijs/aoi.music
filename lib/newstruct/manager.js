...
class Manager extends index_1.TypedEmitter {
    constructor(config) {
        ...
        if (config.searchOptions?.youtubeToken) {
            ...
            async function generateYoutubePoToken() {
                const visitorData = youtubei_js_1.Proto.encodeVisitorData(youtubei_js_1.Utils.generateRandomString(11), Math.floor(Date.now() / 1000));
                const dom = new jsdom_1.JSDOM();
                globalThis.window = dom.window;
                globalThis.document = dom.window.document;
                const bgConfig = {
                    fetch: (url, options) => (0, undici_1.fetch)(url, options),
                ...
                return bgutils_js_1.BG.Challenge.create(bgConfig)
                    ...
                    return bgutils_js_1.BG.PoToken.generate({
        ...
        if (config.searchOptions?.youtubeAuth === true) {
            this.platforms.youtube.then(async (yt) => {
                ...
                yt.session.on("auth", ({ credentials }) => {
                    ...
                    (0, fs_1.writeFileSync)(authPath, JSON.stringify(authCredentials));
    ...
    async test(yt) {
        ...
        if (!info)
            throw new Error("Failed to get video info, possibly PoToken issue");
        const audioStreamingURL = info
            .chooseFormat({ quality: "best", type: "audio" })
            .decipher(yt.session.player);
        if (!audioStreamingURL)
            throw new Error("Failed to get audio streaming URL");
