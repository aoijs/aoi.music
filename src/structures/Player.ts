...
class Player
{
  ...
  join ( channel: VoiceChannel )
  {
    this.voiceState.connection = joinVoiceChannel( {
      ...
      adapterCreator: <DiscordGatewayAdapterCreator> <unknown> channel.guild.voiceAdapterCreator,
  ...
  async _destroyPlayer ()
  {
    ...
    {
      if ( existsSync( `.${ this.manager.config.cache.directory }/${ this.textChannel.guildId }` ) )
      {
        await rm(
          `.${ this.manager.config.cache.directory }/${ this.textChannel.guildId }`,
          {
            recursive: true,
            force: true,
          },
