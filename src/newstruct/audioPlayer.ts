...
export class AudioPlayer {
  ...
  async play(emit = true) {
    ...
    if (this.#modes.ytMix) {
      if (
        this.queue[this.#modes.currentTrack].id === this.#modes.ytMix.lastUrl
      ) {
        const tracks = <any[]>(
  ...
  _destroy() {
    ...
    this.options.manager.players.delete(
      this.options.connection.joinConfig.guildId
    );
    if (this.options.manager.plugins.has(PluginName.Cacher)) {
      const cacher = <Plugin<PluginName.Cacher>>(
        this.options.manager.plugins.get(PluginName.Cacher)
    ...
    for (let i = 0; i < track.length; i++) {
      if (type === PlatformType.Youtube) {
        ...
        const info = await requestInfo(
          track[i],
          formattedPlatforms[PlatformType.SoundCloud],
          this.options.manager
        );
        ...
        let info = <SpotifyTrackInfo[]>(
          (<unknown>(
            await requestInfo(track[i], "Spotify", this.options.manager)
          ))
        );
        if (!info) continue;
        if (!Array.isArray(info)) info = [info];
        for (let i = 0; i < info.length; i++) {
          const moreinfo = await this.options.manager.platforms.spotify.getData(
            info[i].url
          );
          this.queue.push({
            ...info[i],
          ...
          this.options.manager.emit(
            PlayerEvents.TrackAdd,
            <YoutubeTrackInfo | SpotifyTrackInfo>(<unknown>info),
            this
          );
          if (this.queue.length === 1) {
            this.options.manager.emit(PlayerEvents.QueueStart, track, this);
        ...
        const info = await requestInfo(
          track[i],
          formattedPlatforms[PlatformType.Url],
          this.options.manager
        );
        this.queue.push({
          ...(<UrlTrackInfo>info),
  ...
  async autoPlayNext() {
    if (
      ...
      if (
        this.currentTrack.formattedPlatforms.toLowerCase() ===
        AutoPlay.Youtube /*|| this.currentTrack.formattedPlatforms.toLowerCase() === AutoPlay.Spotify*/
      ) {
        const parsed = await (
          await fetch(`https://youtube.com/watch?v=${this.currentTrack.id}`)
        ).text();
        ...
        for (const id of ids) {
          const info = await requestInfo(
            id,
            this.currentTrack.formattedPlatforms.toLowerCase() === "youtube"
              ? "Youtube"
              : "Spotify",
            this.options.manager
          );
      ...
      async (
        ...
        if (newState.status === VoiceConnectionStatus.Disconnected) {
          if (
             ...
             */
            await setTimeout(
              (this.options.connection.rejoinAttempts + 1) * 5_000
            );
