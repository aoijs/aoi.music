...
export async function search(query: string, type: PlatformType, manager: Manager) {
    if (type === PlatformType.Youtube) {
        if (!query.startsWith("https://")) {
            ...
            return [`https://youtube.com/watch?v=${searched.videos[0].as(YTNodes.Video).id}`];
        } else {
            if (query.includes("watch?v=") && query.includes("list=")) {
                ...
                return playlist.videos.as(YTNodes.PlaylistVideo).map((video) => `https://youtube.com/watch?v=${video.id}`);
        ...
        if (query.startsWith("https://")) {
            if (query.includes("https://m.soundcloud.com")) query = query.replace(`https://m.soundcloud.com`, `https://soundcloud.com`);
            return [query];
        }
        ...
        if (existsSync(query)) {
            ...
            if (stats.isDirectory()) {
                ...
                return files
                    .filter((x) => {
                        return x.endsWith(".mp3") || x.endsWith(".wav") || x.endsWith(".ogg") || x.endsWith(".flac");
                    })
                    .map((x) => `${query}/${x}`);
            } else {
                if (query.endsWith(".mp3") || query.endsWith(".wav") || query.endsWith(".ogg") || query.endsWith(".flac")) {
                    return [query];
                }
                return [];
            }
